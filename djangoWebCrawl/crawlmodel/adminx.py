...
from .models import kikuumodel,snatchermodel,fixmodel,jjshousemodel,lovelywholesalemodel,\
    wholesale7model, kilimallmodel, dealhubmodel, valuecomodel
...
class baseAdmin(object):
    ...
    #可供过滤的属性
    list_filter=['huitou','saledCount','service','wwxy','cfmj','zrs','num_comment','good_percent','rateAverageStarLevel',
                 'deliverySpeed', 'date']
    #可跳转的链接
...
class lovelywholesaleAdmin(baseAdmin):
    # import_export_args = {'import_resource_class': lovelywholesaleResource, 'export_resource_class': lovelywholesaleResource}
    import_export_args = {'import_resource_class': lovelywholesaleResource}


class wholesale7Admin(baseAdmin):
    # import_export_args = {'import_resource_class': wholesale7Resource, 'export_resource_class': wholesale7Resource}
